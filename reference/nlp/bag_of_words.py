...
texts = [' '.join(text.split()) for text in texts]                                              # trim spaces

# histogram of text length
text_lengths = [len(text.split()) for text in texts]    # length of sentence
text_lengths = [x for x in text_lengths if x < 50]
plt.hist(text_lengths, bins=10)
plt.title('Histogram of # of words in texts')
plt.show()

sentence_size = 25  # choosen from histogram
min_word_freq = 3
...
# train_test_split
train_indices = np.random.choice(len(texts), round(len(texts) * 0.8), replace=False)    # 80% go in train
test_indices = np.array(list(set(range(len(texts))) - set(train_indices)))              # rest in test
