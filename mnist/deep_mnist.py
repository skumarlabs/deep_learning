...
with tf.name_scope('input'):
    input_images = tf.constant(X_train_pca)
    input_labels = tf.constant(y_train)
...
with tf.Session(config=config) as sess:
    ...
    # saver.restore(sess, 'checkpoints/my_model-3300')
    ...
    print('total examples: ', len(X_train_pca))
    epoch_num = 0
    step = 1
    try:
        while not coord.should_stop():
            ...
            if step % 100 == 0:
                ...
                print('saving model state at {:d}'.format(step))
                saver.save(sess, 'checkpoints/my_model', global_step=step)

            if step % max_batch_num == 0:
                # increment epoch_num
                epoch_num += 1
                print('epoch {:d} completed'.format(epoch_num))
            step += 1
    except tf.errors.OutOfRangeError:
        saver.save(sess, 'checkpoints/my_model', global_step=step - 1)
        print('training finished at step:', step - 1)
    finally:
        coord.request_stop()
    coord.join(threads)
